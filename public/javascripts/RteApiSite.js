...
function Site_CreateLogger() {
  ...
  if (typeof this.Logger == 'undefined' || this.Logger == null) {
    ...
    var RTE_appender = new log4javascript.InPageAppender(
      'myLogger',
      false,
      false,
      true,
...
// Reinitialize object removes all current state from memory.
function Site_Reset1() {
  this.m_aValues = new Array(); // holds complete data model
  this.m_aChangedValues = new Array(); // holds only elements that have changed in Values array.
  this.m_validNavigationCommands = new Array();
  this.m_errorManager = new ErrorMessageManager(this.Logger);
...
function Site_ClearValidNavigationCommands() {
  for (i in this.m_validNavigationCommands) this.m_validNavigationCommands[i] = null;
}
...
function Site_SetMasteryLevel(strMasteryLevel) {
  if (this.Validator.Validate(strMasteryLevel, 'CmiDecimal0-100OrBlank')) {
    ...
    this.Logger.error(
      String.format(
        'Mastery Score not set, value is not in range 0-100 or blank: {0}',
        strMasteryLevel
      )
    );
  }
...
function Site_SetDataFromLMS(strDataFromLMS) {
  if (this.Validator.Validate(strDataFromLMS, 'CmiString4096')) {
    ...
    this.Logger.error(
      String.format('Data From LMS not set, value is not valid: {0}', strDataFromLMS)
    );
  }
}
...
function ErrorMessageManager(logger) {
  ...
  this.SetError = function(errorCode, errorMessageId, param1, param2, param3) {
    ...
    lastDiagnosticMessage = lastDiagnosticMessage.replace(
      /\{0\}/g,
      param1 == null ? defaultParameterValue : param1
    );
    lastDiagnosticMessage = lastDiagnosticMessage.replace(
      /\{1\}/g,
      param2 == null ? defaultParameterValue : param2
    );
    lastDiagnosticMessage = lastDiagnosticMessage.replace(
      /\{2\}/g,
      param3 == null ? defaultParameterValue : param3
    );
  ...
  this.GetErrorString = function(errorId) {
    ...
    if (errorId == lastError) return lastErrorMessage;
  ...
  this.GetDiagnostic = function(errorCode) {
    if (errorCode == null || errorCode == '' || errorCode == lastError) {
      ...
      if (errorCode != null && errorCode != '') return this.GetErrorString(errorCode);
    }
    return '';
  };
  ...
  function InitializeDescriptions_12() {
    ...
    var L_ERROR1014_TXT =
      'General exception error. The type of the data model element {0} cannot be determined.';
    ...
    var L_ERROR2013_TXT =
      'General argument error. The given data model name must be a string type.';
    var L_ERROR2014_TXT = 'General argument error. The value cannot be null or undefined.';
    var L_ERROR2015_TXT =
      "The index of data model element '{0}' is expected to be smaller than the value of '{1}', which is '{2}'.";
    var L_ERROR2016_TXT =
      "The index of data model element '{0}' is expected to be smaller than or equal to the value of '{1}', which is '{2}'.";
